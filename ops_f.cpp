...
void mul(float *__restrict__ left, float *__restrict__ right, float *__restrict__ output, int length)
{
    ...
    int nstep = length / 32;
    int leftover = length & 31;
    for (int i = 0; i < nstep; i++)
    {
        HVX_Vector out = Q6_Vqf32_vmpy_VsfVsf(hleft[i], hright[i]);
        out = Q6_Vsf_equals_Vqf32(out);
        vmemu(&houtput[i]) = out;
    }
    ...
    if (leftover > 0)
    {
        ...
        HVX_Vector out = Q6_Vqf32_vmpy_VsfVsf(hleft[nstep], hright[nstep]);
        out = Q6_Vsf_equals_Vqf32(out);
        Q6_vmem_QnRIV(ql_not, (HVX_Vector *)(houtput + nstep), out);
    }
}
...
void mul(float *__restrict__ input, float scalar, float *__restrict__ output, int length)
{
    ...
    for (int i = 0; i < nstep; i++)
    {
        HVX_Vector out = Q6_Vqf32_vmpy_VsfVsf(hinput[i], scalar_vec);
        out = Q6_Vsf_equals_Vqf32(out);
        vmemu(&houtput[i]) = out;
    }
    ...
    if (leftover > 0)
    {
        ...
        HVX_Vector out = Q6_Vqf32_vmpy_VsfVsf(hinput[nstep], scalar_vec);
        out = Q6_Vsf_equals_Vqf32(out);
        Q6_vmem_QnRIV(ql_not, (HVX_Vector *)(houtput + nstep), out);
    }
}
